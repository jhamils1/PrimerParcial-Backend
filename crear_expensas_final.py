...
def generar_expensas_sql():
    ...
    # Eliminar expensas existentes
    count = expensa.objects.count()
    if count > 0:
        print(f"Eliminando {count} expensas existentes...")
    ...
    with connection.cursor() as cursor:
        for idx, cont in enumerate(contratos):
            ...
            print(f"Unidad {cont.unidad.codigo} - Inicio: {fecha_inicio_contrato.strftime('%b %Y')}")
            ...
            while fecha_expensa <= fecha_actual:
                ...
                """, [
                    ...
                    fecha_venc,
                    pagada,
                    f"Expensa {fecha_expensa.strftime('%B %Y')}",
                    "usd"
                ])
            ...
            print(f"  ✓ {expensas_unidad} expensas\n")
    
    # Resumen
    print(f"{'='*60}")
    print("RESUMEN:")
    print(f"  • Total expensas: {total_expensas_creadas}")
    print(f"  • Pagadas: {expensas_pagadas} ({expensas_pagadas*100//total_expensas_creadas}%)")
    print(f"  • Pendientes: {total_expensas_creadas - expensas_pagadas} ({(total_expensas_creadas - expensas_pagadas)*100//total_expensas_creadas}%)")
    print(f"{'='*60}")
    ...
    for year in [2022, 2023, 2024, 2025]:
        ...
        print(f"  • {year}: {count} ({pagadas_year} pagadas, {count-pagadas_year} pendientes)")
